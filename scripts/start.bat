...
REM Health check: the venv may exist but hold a STALE research.exe (e.g. built
REM against an old package name after a refactor). So we don't just test that
...
REM The package `src` lives in the project root (parent of this scripts dir),
...
REM ---------- P1: anchor/de-anchor (helps person topics with institution names) ----------
echo.
echo Optional: P1 two-phase anchor/de-anchor. Useful when topic mixes
echo institution + person name (e.g. "Tsinghua Zhang Wei"). Enter the
echo person/concept as core, and dimension tags as facets.
set "core="
set /p core=Core keyword (optional, Enter to skip):
set "facets="
set /p facets=Facets comma-separated (optional, e.g. PhD,papers,NTU):
set "P1FLAGS="
if not "!core!"=="" set "P1FLAGS=!P1FLAGS! --core "!core!""
if not "!facets!"=="" set "P1FLAGS=!P1FLAGS! --facets "!facets!""
REM ---------- P2: quality boosters (deep-search / relevance / backward) ----------
echo.
echo Optional P2 quality boosters (each adds time + LLM cost):
set "ds="
set /p ds=Deep-search multi-sort multi-page? (y/N):
set "P2FLAGS="
if /i "!ds!"=="y" set "P2FLAGS=!P2FLAGS! --deep-search"
set "rf="
set /p rf=LLM relevance filter (drop low-relevance docs)? (y/N):
if /i "!rf!"=="y" set "P2FLAGS=!P2FLAGS! --relevance-filter"
set "br="
set /p br=Backward-pass rounds 0-3 (Enter = 0):
if not "!br!"=="" if not "!br!"=="0" set "P2FLAGS=!P2FLAGS! --max-backward-rounds !br!"
echo.
echo Researching: !topic!   source !SRC!   rounds !rounds!  (LLM query expansion on)
research run "!topic!" !SRC! -r !rounds! --llm-expand !MCMD! !QFLAGS! !P1FLAGS! !P2FLAGS! --skip extract
echo.
...
echo    research run "Inst PersonName" --core "PersonName" --facets "PhD,papers"
echo    research run "topic" --deep-search --relevance-filter --max-backward-rounds 1
