...
from research_tool.ingest.pdf import PdfIngestor
from research_tool.llm import MockLLMClient
from research_tool.models import PdfIngestConfig
...
def fake_mineru(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("research_tool.ingest.pdf.shutil.which", lambda c: "/usr/bin/mineru")
    ...
    monkeypatch.setattr("research_tool.ingest.pdf.subprocess.run", fake_run)
...
async def test_missing_mineru_clear_error(tmp_path, monkeypatch):
    monkeypatch.setattr("research_tool.ingest.pdf.shutil.which", lambda c: None)
    _make_pdf(tmp_path)
