...
async def test_search_warnings_not_silent(monkeypatch):
    ...
    async def ddg_ok(self, q, n, lang):
        return [SearchHit(url="https://a.com", source_engine="web")]

    async def gh_boom(self, q, n, lang):
        raise SearchError("github 搜索失败: 429")

    monkeypatch.setattr(
        "research_tool.search.duckduckgo.DuckDuckGoBackend.search", ddg_ok
    )
    monkeypatch.setattr(
        "research_tool.search.github_backend.GitHubBackend.search", gh_boom
    )
