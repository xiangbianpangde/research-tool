...
from research_tool.stages.collector import (
    Collector,
    _build_queries,
    _llm_build_queries,
)
...
async def test_collector_uses_template_without_llm(monkeypatch):
    ...
    async def fake_search(self, query, max_results, language):
        captured.setdefault("queries", []).append(query)
        return []

    monkeypatch.setattr(
        "research_tool.search.duckduckgo.DuckDuckGoBackend.search", fake_search
    )
