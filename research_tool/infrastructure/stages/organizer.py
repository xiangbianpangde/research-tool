...
_FB_SYSTEM = "你是调研质量裁判，发现知识树的薄弱节点/断层/矛盾，产出补充检索查询。"
...
class Organizer:
    ...
    async def assess_and_feedback(
        ...
        失败回退：LLM 异常 → 仅返回稀疏节点列表，queries 为空（pipeline 据此终止反向）。
        """
        min_ev = self.config.min_evidence_per_node
        sparse: list[str] = []
        node_titles: list[str] = []
        bodies: list[str] = []
        for path in result.nodes:
            text = path.read_text(encoding="utf-8")
            # 节点标题：N1 后空格的剩余部分；fallback 用文件名
            head = text.splitlines()[0] if text else path.stem
            title = head.lstrip("# ").split(" ", 1)[-1].strip() or path.stem
            node_titles.append(title)
            bodies.append(text)
            if len(_SOURCE_REF.findall(text)) < min_ev:
                sparse.append(title)

        if not sparse and len(node_titles) < 2:
            return FeedbackPlan()

        # 单次 LLM 调用同时处理稀疏/断层/矛盾，节省成本
        ...
        prompt = (
            ...
            f"{sparse_hint}"
            "请基于节点标题与上述提示，产出**补充检索查询**以提升知识树质量："
            "(1) 给稀疏节点补充更多证据；"
            "(2) 桥接两个相关但当前缺少连接的节点（如果存在）；"
            "(3) 对节点间疑似矛盾的内容生成交叉验证查询（如果存在）。\n"
            "查询应是可直接送搜索引擎的短语，避免太长太宽。\n"
            "返回 JSON：{queries:[...], notes:[简要说明每条查询的目的]}。"
        )
        try:
        ...
        queries = [q.strip() for q in fb.queries if q.strip()]
