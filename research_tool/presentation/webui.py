...
def _find_repo_root(start: Path | None = None) -> Path | None:
    """寻找仓库根目录（含 .git 或 pyproject.toml）。"""
...
def is_safe_path(base_dir: Path | str, target_path: Path | str) -> bool:
    """校验 target_path 是否安全且不会逃逸出 base_dir 范围（防路径穿越与空字节注入）。"""
    try:
        if isinstance(target_path, str) and "\x00" in target_path:
            return False
        if isinstance(base_dir, str) and "\x00" in base_dir:
            return False
...
def _assert_safe_output_dir(target_dir: Path | str | None) -> None:
    """Zero Workspace Mutation 硬防护与路径安全校验：
    禁止包含空字节、路径穿越逃逸，禁止将实际落盘目录指定为项目根目录或仓库源码/文档/测试目录。
    """
    if target_dir is None:
        return
    raw_str = str(target_dir)
    if "\x00" in raw_str:
        raise ValueError("安全拦截：输出路径包含非法空字节（Null Byte）")

    target_path = Path(target_dir)
    resolved_out = target_path.resolve()
    repo_root = _find_repo_root()
    if repo_root is not None:
        if resolved_out == repo_root:
            raise ValueError(f"安全拦截：输出目录不能直接指向项目根目录（{repo_root}），请指定子目录（如 ./research-output）")
        if resolved_out.is_relative_to(repo_root):
            ...
            if top_part not in allowed_dirs:
                raise ValueError(
                    f"安全拦截（Zero Workspace Mutation）：禁止向仓库源码/资产/文档目录（{top_part}）输出调研产物！请指定 ./research-output"
                )
    else:
        if (resolved_out / "research_tool").exists() or (resolved_out / ".git").exists():
            raise ValueError("安全拦截：输出目录不能直接指向项目根目录")
...
def _run_threaded_video_notes(
    ...
    """视频笔记后端：调 VideoPipeline → 落盘 → 可选原生调研管道。"""
    ...
    async def go():
        ...
        for extra_fname in ("sources.json", "run-summary.json", "state.json"):
            extra_file = topic_dir / extra_fname
            if extra_file.exists():
                files.append(str(extra_file))
...
def _list_history_videos(work_dir: str) -> list[str]:
    ...
    if "\x00" in (work_dir or ""):
        return []
    try:
        _assert_safe_output_dir(work_dir)
...
def run_video_note(  # noqa: PLR0915 - Gradio 流式生成器，单函数承载多状态推送
    ...
    if not topic:
        ...
        return
    if "\x00" in topic:
        yield "⚠ 安全拦截：调研主题包含非法字符（空字节）。", "", None, ""
        return
    if "\x00" in work_dir:
        yield "⚠ 安全拦截：输出路径包含非法空字节（Null Byte）。", "", None, ""
        return
    try:
        _assert_safe_output_dir(work_dir)
        _assert_safe_output_dir(Path(work_dir) / slugify(topic))
    ...
    log: list[str] = [
        ...
        "流程：下载 → 转写 → AI 笔记 → 落盘" + (" → 九段闭环调研报告" if also_make_report else ""),
        "—" * 20,
    ]
...
def _stage_cn(stage: str) -> str:
    """阶段标识转换为规范中文显示名，支持别名与回退。"""
    canon = _STAGE_ALIASES.get(stage, stage)
    return _STAGE_CN.get(canon, _STAGE_CN.get(stage, stage))
...
def run_web(  # noqa: PLR0915 - Gradio 事件处理，单函数承载多模式分支
    ...
    **kwargs,
    ...
    work_dir_str = (work_dir or "./research-output").strip()
    if "\x00" in work_dir_str:
        yield "⚠ 安全拦截：输出路径包含非法空字节（Null Byte）。", "", None, ""
        return
    try:
        _assert_safe_output_dir(work_dir_str)
        _assert_safe_output_dir(Path(work_dir_str) / slugify(topic))
    except (ValueError, RuntimeError) as e:
        yield f"⚠ 安全拦截：{e}", "", None, ""
        return

    if mode == "PDF 调研":
        raw_pdf = (pdf_path or "").strip()
        if not raw_pdf:
            yield "⚠ PDF 模式需填写 PDF 文件或文件夹路径。", "", None, ""
            return
        if "\x00" in raw_pdf:
            yield "⚠ 安全拦截：PDF 路径包含非法字符（空字节）。", "", None, ""
            return

    # Agent Strict 安全审查校验
    is_strict_agent = os.environ.get("RESEARCH_AGENT_STRICT", "0").strip().lower() in ("1", "true", "yes")
    if is_strict_agent:
        if mode == "brief":
            yield "⚠ 安全拦截：当前环境开启了 RESEARCH_AGENT_STRICT，严禁使用 --mode brief 偷懒缩水，必须执行完整九段闭环管线！", "", None, ""
            return
        if not do_extract:
            yield "⚠ 安全拦截（RESEARCH_AGENT_STRICT）：严禁缩水或跳过核心阶段！缺失阶段：['extract']。Agent 调研必须执行完整九段闭环管线。", "", None, ""
            return
        if int(qgate_max_high) > 0:
            logger.warning("Agent Strict 告警：当前开启了严格模式，但 QGate 允许高危矛盾数 > 0 (%s)", qgate_max_high)
    ...
    if not do_extract and "extract" in stages:
    ...
    overrides: dict = {
        ...
        "inspect": {
            "enabled": True,
            "rules": ["contradiction", "orphan_node", "span_incomplete"],
            "priority_filter": ["high", "medium", "low"],
        },
        "targeted": {
            "max_rounds": int(max_targeted_rounds),
            "max_queries_per_round": int(max_targeted_queries),
            "max_results_per_query": 5,
        },
        "qgate": {
            "max_high_findings": int(qgate_max_high),
            "max_total_findings": int(qgate_max_total),
        },
    }
    if cleaner_cfg:
    ...
    stage_display = " → ".join(_stage_cn(s) for s in stages)
    log: list[str] = [f"主题：{topic}", f"流程：{stage_display}", "—" * 20]
...
def build_ui():  # noqa: PLR0915 - Gradio 布局 + 事件绑定，单函数承载 UI 组装
    ...
    with gr.Blocks(title="research-tool 调研 + 视频笔记") as app:
        gr.Markdown(
            "# 🔍 research-tool\n"
            "**Tab 1 调研**：原生九段闭环架构（缺口检视、靶向补搜、CAS增量合并、质量门控），自动产出知识树 / 调研报告。  \n"
            "**Tab 2 视频笔记**：粘贴 B 站 / YouTube 链接，AI 自动转写并生成结构化 Markdown 笔记。"
        )

        with gr.Tabs():
            # ========================================================== #
            # Tab 1: 调研（原生九段闭环架构 UI）
            # ========================================================== #
            with gr.Tab("🔍 调研"):
                with gr.Row():
                    # -------- 左栏：输入与设置 -------- #
                    with gr.Column(scale=1, min_width=380):
                        ...
                        do_extract = gr.Checkbox(
                            label="启用实体/三元组抽取（更慢，更细）", value=True
                        )

                        with gr.Accordion("质量门控与靶向自愈（闭环控制）", open=False):
                            with gr.Row():
                                qgate_max_high = gr.Slider(
                                    0,
                                    5,
                                    value=0,
                                    step=1,
                                    label="QGate 允许矛盾数（0=零容忍）",
                                )
                                qgate_max_total = gr.Slider(
                                    0,
                                    30,
                                    value=10,
                                    step=1,
                            ...
                            with gr.Row():
                                ...
                                max_targeted_queries = gr.Slider(
                                    1,
                                    30,
                                    value=10,
                                    step=1,
                    ...
                    with gr.Column(scale=2, min_width=520):
                        ...
                        files = gr.File(label="下载（报告 + 知识树节点 + 来源清单）", file_count="multiple")
