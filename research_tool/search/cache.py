...
class SearchCache:
    ...
    def _path(self, engine: str, query: str, max_results: int, language: str) -> Path:
        raw = f"{engine}|{query}|{max_results}|{language}"
    ...
    def get(
        ...
        path = self._path(engine, query, max_results, language)
        if not path.exists():
            return None
        try:
...
class CachingBackend(SearchBackend):
    ...
    async def search(
        ...
        cached = self.cache.get(self.name, query, max_results, language)
        if cached is not None:
            return cached
        hits = await self.inner.search(query, max_results, language)
        if hits:
            self.cache.set(self.name, query, max_results, language, hits)
        return hits
